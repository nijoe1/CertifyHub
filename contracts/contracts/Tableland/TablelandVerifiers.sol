...
contract TablelandVerifiers is Ownable {
  ...
  string private constant COMPANY_SCHEMA =
    "company text, image text, description text, admin text";

  string private constant EVENT_TABLE_PREFIX = "event";
  string private constant EVENT_SCHEMA =
    "company text, eventID text, cid text, startsAt text, endsAt text";

  string private constant EVENT_VERIFIERS_TABLE_PREFIX = "event_verifiers";
  string private constant EVENT_VERIFIERS_SCHEMA =
    "eventID text, verifierAddress text";
  ...
  constructor() {
    ...
    tables.push(
      SQLHelpers.toNameFromId(EVENT_VERIFIERS_TABLE_PREFIX, tableIDs[2])
    );
  }

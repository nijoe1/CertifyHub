...
export const getCompaniesVerifier = async (address) => {
  ...
  FROM ${tables.company} AS C, ${tables.company_event} AS A  JOIN  ${
    tables.company_event_verifiers
  } AS B ON A.eventID = B.eventID WHERE B.verifierAddress='${address.toLowerCase()}' AND A.company = C.company`;
  const categories = await fetch(query);
  let result = await categories.json();
  return result;
};
